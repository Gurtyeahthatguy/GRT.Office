...
fn read_grt(path: String) -> Result<serde_json::Value, String> {
    ...
    for entry in entries {
        // Lossy on purpose: a damaged part should surface as visible
        // replacement characters rather than refuse the whole document.
        let text = String::from_utf8_lossy(&entry.data).into_owned();
        parts.insert(entry.name, serde_json::Value::String(text));
    }
